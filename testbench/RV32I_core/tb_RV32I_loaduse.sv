...
//   x3 = DMEM[0]
//   x5 = x3 (same value)
// Also checks: no trap, no unexpected stores.
//
// NOTE: If your core still has the "first instruction at PC=0 gets skipped" issue,
// insert a NOP at imem[0] (I include it by default below). If you implement Fetch Option A,
// you can remove that initial NOP.

module tb_rv32i_load_use;
  ...
  // add x5,x3,x0     = 0x000182B3   (funct7=0 rs2=0 rs1=3 funct3=0 rd=5 opcode=0x33)
  // ebreak           = 0x00100073
  integer ii;
  initial begin
    ...
    // NOP padding (remove if you implemented Fetch Option A)
    imem[0] = 32'h0000_0013; // NOP
    imem[1] = 32'h0000_2183; // lw x3,0(x0)
    imem[2] = 32'h0001_82B3; // add x5,x3,x0
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      // No stores expected in this test
      if (dmem_we) begin
        $display("[FAIL] Unexpected store: addr=0x%08x wdata=0x%08x wstrb=%b @ t=%0t",
      ...
      if (dut.rf_we) begin
        if (dut.rf_waddr == 5'd3) begin
          ...
          if (dut.rf_wdata !== EXPECT) begin
            $display("[FAIL] x3 <= 0x%08x expected 0x%08x @ t=%0t",
        ...
        if (dut.rf_waddr == 5'd5) begin
          x5_seen <= 1'b1;
          if (dut.rf_wdata !== EXPECT) begin
            $display("[FAIL] x5 <= 0x%08x expected 0x%08x @ t=%0t",
                     dut.rf_wdata, EXPECT, $time);
            $fatal;
          end else begin
            $display("[OK] x5 <= 0x%08x", dut.rf_wdata);
          end
        end
      end

      if (cycle_count > 250) begin
        $display("[FAIL] Timeout (>250 cycles). x3_seen=%0b x5_seen=%0b done_o=%0b",
                 x3_seen, x5_seen, done_o);
        $fatal;
      end
    end
  end
  ...
  initial begin
    ...
    wait (x3_seen && x5_seen);

    $display("[PASS] Load-use hazard handled: x3 and x5 both 0x%08x", EXPECT);

    // optional: observe done_o for a few cycles
